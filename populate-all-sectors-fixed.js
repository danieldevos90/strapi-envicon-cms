/**
 * Populate All Sectors with Content from text.md - Fixed Version
 * Uses Strapi Entity Service API to properly handle nested components
 */
...
console.log('🚀 Populating all sectors with content from text.md (Fixed Version)...');
...
async function populateSector(slug, data) {
  try {
    ...
    // Prepare sectorContent - only include features if it has items
    ...
    // Prepare sectorAccordions - only include accordions if it has items
    ...
    // Update in multiple steps to work around Strapi v5 nested component limitations
    console.log(`🔄 Updating sector...`);
    
    // Step 1: Update root fields and sectorContent (without nested arrays)
    const step1Data = {
      data: {
        ...
        sectorFeatures: {
          title: data.sectorFeatures.title
        },
        sectorAccordions: {
          title: data.sectorAccordions.title,
          description: data.sectorAccordions.description,
    ...
    console.log(`  Step 1: Updating base components...`);
    const step1Response = await axios.put(`${API_URL}/sectors/${documentId}`, step1Data);
    if (step1Response.data?.error) {
      throw new Error(step1Response.data.error.message || 'Step 1 failed');
    }
    console.log(`  ✅ Step 1 complete`);
    
    // Step 2: Update sectorFeatures with features array
    if (data.sectorFeatures.features && data.sectorFeatures.features.length > 0) {
      console.log(`  Step 2: Adding ${data.sectorFeatures.features.length} features...`);
      // Use PATCH to update just the features array
      const step2Data = {
        data: {
          sectorFeatures: {
            title: data.sectorFeatures.title,
            features: data.sectorFeatures.features
          }
        }
      };
      
      const step2Response = await axios.put(`${API_URL}/sectors/${documentId}`, step2Data);
      if (step2Response.data?.error) {
        console.warn(`  ⚠️ Step 2 warning: ${step2Response.data.error.message}`);
        // Don't throw - features might need manual addition
      } else {
        console.log(`  ✅ Step 2 complete`);
      }
    }
    
    // Step 3: Update sectorAccordions with accordions array
    if (data.sectorAccordions.accordions && data.sectorAccordions.accordions.length > 0) {
      console.log(`  Step 3: Adding ${data.sectorAccordions.accordions.length} accordions...`);
      const step3Data = {
        data: {
          sectorAccordions: {
            title: data.sectorAccordions.title,
            description: data.sectorAccordions.description,
            accordions: data.sectorAccordions.accordions,
            ...(data.sectorAccordions.imageAlt && { imageAlt: data.sectorAccordions.imageAlt })
          }
        }
      };
      
      const step3Response = await axios.put(`${API_URL}/sectors/${documentId}`, step3Data);
      if (step3Response.data?.error) {
        console.warn(`  ⚠️ Step 3 warning: ${step3Response.data.error.message}`);
        // Don't throw - accordions might need manual addition
      } else {
        console.log(`  ✅ Step 3 complete`);
      }
    }
    
    console.log(`✅ Sector ${slug} updated!`);
    
    // Verify basic structure
    try {
      const verifyResponse = await axios.get(`${API_URL}/sectors?filters[slug]=${slug}&populate=sectorContent&populate=sectorFeatures&populate=sectorAccordions`);
      const updatedSector = verifyResponse.data.data[0];
      
      console.log(`📊 Verification:`);
      console.log(`  - SectorContent: ${updatedSector.attributes?.sectorContent ? '✅' : '❌'}`);
      console.log(`  - SectorFeatures: ${updatedSector.attributes?.sectorFeatures ? '✅' : '❌'}`);
      console.log(`  - SectorAccordions: ${updatedSector.attributes?.sectorAccordions ? '✅' : '❌'}`);
      
      // Check nested arrays
      const featuresCount = updatedSector.attributes?.sectorFeatures?.features?.length || 0;
      const accordionsCount = updatedSector.attributes?.sectorAccordions?.accordions?.length || 0;
      
      if (featuresCount === 0 && data.sectorFeatures.features?.length > 0) {
        console.log(`  ⚠️ Features not saved - may need manual addition via Strapi admin`);
      } else if (featuresCount > 0) {
        console.log(`  ✅ Features: ${featuresCount} items`);
      }
      
      if (accordionsCount === 0 && data.sectorAccordions.accordions?.length > 0) {
        console.log(`  ⚠️ Accordions not saved - may need manual addition via Strapi admin`);
      } else if (accordionsCount > 0) {
        console.log(`  ✅ Accordions: ${accordionsCount} items`);
      }
    } catch (verifyError) {
...
async function populateAll() {
  ...
  console.log('💡 Note: If features/accordions show 0 items, they may need to be added manually via Strapi admin');
  console.log('💡 This is a known limitation with Strapi v5 nested repeatable components via REST API');
}
